...
function AgentLogo(): ReactElement {
  return (
    <Link
...
function AgentNavigation({
  mobile = false,
  onNavigate,
}: AgentNavigationProps): ReactElement {
  ...
  return (
    <nav aria-label="Agent navigation">
      ...
      >
        {AGENT_NAV_ITEMS.map(({ label, href, icon: Icon }) => {
...
export default function AgentHeader({
  ...
  const profileInitials = user
    ...
    : "A";
  const isSettingsActive = pathname.startsWith("/agent/settings");
