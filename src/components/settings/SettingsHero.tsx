...
export default function SettingsHero({ onViewProfile }: SettingsHeroProps) {
  ...
  const initials = user
    ...
    : profile.initials;
  const displayName = user?.firstName || profile.name;
  const email = user?.email ?? profile.email;
  const identityVerified = user?.identityVerified ?? false;
  const showTenantVerifiedBadge =
    role === "tenant" && (identityVerified || isTenantVerified(state));
    ...
    <section className="grid overflow-hidden rounded-b-xl bg-primary text-white lg:grid-cols-[8rem_1fr_19rem] lg:items-center">
      ...
      <div className="px-6 py-8 sm:px-8 lg:px-6 lg:py-12">
        ...
        {role === "tenant" ? (
          ...
          <div className="mt-6 inline-flex items-center rounded-full bg-white/10 px-3 py-1.5 font-body text-xs font-medium text-white">
