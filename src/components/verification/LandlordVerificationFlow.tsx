...
type KybStage = "profile" | "documents" | "review";
type JourneyScreen =
  | "overview"
  | "kyb"
  | "payout"
...
export default function LandlordVerificationFlow(): ReactElement | null {
  ...
  const [kybStage, setKybStage] = useState<KybStage>("profile");
  ...
  const businessIdentityComplete =
    businessType === "REGISTERED"
      ? legalBusinessName.trim().length > 0 &&
        registrationNumber.trim().length > 0
      : businessType === "INDIVIDUAL" && fullLegalName.trim().length > 0;
  const addressComplete =
    businessType === "REGISTERED"
      ? businessAddress.trim().length > 0
      : businessType === "INDIVIDUAL" && residentialAddress.trim().length > 0;
  const kybReady =
    Boolean(businessType) && businessDetailsValid && hasAllDocuments;
  const kybStageIndex =
    kybStage === "profile" ? 0 : kybStage === "documents" ? 1 : 2;
  const selectedLegalName =
    businessType === "REGISTERED" ? legalBusinessName : fullLegalName;
  const selectedAddress =
    businessType === "REGISTERED" ? businessAddress : residentialAddress;
  const primaryDocumentName =
    documents.find((document) => document.fieldId === "business")?.file.name ??
    "";
  const proofOfAddressName =
    documents.find((document) => document.fieldId === "address")?.file.name ??
    "";
  ...
  const renderKybDossierStep = (): ReactElement => (
    <main className="fixed inset-0 z-[120] overflow-x-hidden overflow-y-auto bg-bg">
      ...
      <div className="mx-auto min-h-screen w-full max-w-5xl px-5 pb-14 pt-28 sm:px-8">
        ...
        <div className="mt-8 grid items-start gap-6 lg:grid-cols-[15rem_minmax(0,1fr)] lg:gap-8">
          <aside className="rounded-xl border border-primary/10 bg-transparent p-5 lg:sticky lg:top-28">
            <h2 className="font-body text-sm font-medium text-primary">
              Verification checklist
            </h2>
            <p className="mt-1 font-body text-xs leading-5 text-muted">
              Complete each requirement before submitting.
            </p>
            ...
            <div className="mt-5 grid gap-4">
              {[
                ...
                <div key={item.label} className="flex items-center gap-3">
                  <span
                    className={cn(
                      "flex h-7 w-7 shrink-0 items-center justify-center rounded-full border font-body text-xs font-medium",
                      item.complete
                        ? "border-accent text-accent"
                        : "border-primary/20 text-muted",
                    )}
                  >
                    {item.complete ? (
            ...
            <div className="mt-6 border-t border-primary/10 pt-5">
              <p className="flex items-start gap-3 font-body text-xs leading-5 text-muted">
                <Clock
              ...
              <p className="mt-3 flex items-start gap-3 font-body text-xs leading-5 text-muted">
                <Lock
                  className="mt-0.5 h-4 w-4 shrink-0 text-accent"
                  aria-hidden="true"
                />
  ...
  const renderKybGuidedStep = (): ReactElement => (
    <main className="fixed inset-0 z-[120] overflow-x-hidden overflow-y-auto bg-bg">
      {renderTopBar(1)}
      <div className="mx-auto min-h-screen w-full max-w-3xl px-5 pb-14 pt-28 sm:px-8">
        <div className="text-center">
          <span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full border border-accent/40 text-accent">
            <ShieldCheck size={22} aria-hidden="true" />
          </span>
          <p className="mt-4 font-body text-xs font-medium uppercase tracking-wide text-muted">
            Landlord verification
          </p>
          <h1 className="mt-1 font-display text-3xl font-bold text-primary">
            Verify your business
          </h1>
          <p className="mx-auto mt-2 max-w-xl font-body text-sm leading-6 text-muted">
            We review a few documents to confirm you&apos;re a legitimate host.
            Our team typically completes this within 1-2 business days.
          </p>
        </div>

        <ol className="mx-auto mt-8 grid max-w-2xl grid-cols-3 gap-2">
          {[
            ["Business profile", 0],
            ["Documents", 1],
            ["Review", 2],
            ...
            const isComplete = stepIndex < kybStageIndex;
            const isActive = stepIndex === kybStageIndex;

            return (
              <li key={label} className="text-center">
                <span
                  className={cn(
                    "mx-auto flex h-8 w-8 items-center justify-center rounded-full border font-body text-xs font-medium",
                    isComplete || isActive
                      ? "border-accent text-accent"
                      : "border-primary/20 text-muted",
                  )}
                ...
                <span
                  className={cn(
                    "mt-2 block font-body text-[11px] font-medium sm:text-xs",
        ...
        <AnimatePresence initial={false} mode="wait">
          <motion.section
            key={kybStage}
            className="mx-auto mt-7 max-w-2xl rounded-xl border border-primary/10 bg-white p-5 shadow-sm sm:p-8"
          ...
          >
            {kybStage === "profile" ? (
              <>
                ...
                <p className="mt-2 font-body text-sm leading-6 text-muted">
                  Choose a profile, then provide the matching legal details.
                </p>
                ...
                <fieldset className="mt-6">
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    {(
                      [
                        {
                          description: "A CAC-registered company or business.",
                          label: "Registered Business",
                          value: "REGISTERED",
                        },
                        {
                          description: "You operate under your own legal name.",
                      ...
                      <button
                        ...
                        className={cn(
                          "relative min-h-24 rounded-xl border bg-transparent p-4 pr-10 text-left transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent",
                          businessType === value
                      ...
                      >
                        {businessType === value ? (
                          <Check
                            className="absolute right-4 top-4 h-4 w-4 text-accent"
                            aria-hidden="true"
                          />
                        ) : null}
                        <span className="block font-body text-sm font-medium text-primary">
                ...
                <button
                  type="button"
                  disabled={!businessType || !businessDetailsValid}
                  onClick={() => {
                    setFormError("");
                    setKybStage("documents");
                  }}
                  className="mt-7 flex min-h-12 w-full items-center justify-center rounded-full border border-primary/15 bg-transparent px-6 py-3 font-body text-sm font-medium text-primary shadow-sm transition-all duration-200 hover:border-accent hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-accent disabled:cursor-not-allowed disabled:opacity-50"
                >
            ...
            {kybStage === "documents" ? (
              <>
                <div className="flex items-center gap-3 rounded-xl border border-primary/10 p-4">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-accent text-accent">
                    <Check size={15} aria-hidden="true" />
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className="block font-body text-xs text-muted">
                    ...
                    <span className="block truncate font-body text-sm font-medium text-primary">
                      {selectedLegalName}
                    </span>
                  </span>
                  <button
                    type="button"
                    onClick={() => setKybStage("profile")}
                    className="font-body text-xs font-medium text-primary underline underline-offset-4 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                  >
                    Edit
                  </button>
                </div>
            ...
            {kybStage === "review" ? (
              <>
                ...
                <p className="mt-2 font-body text-sm leading-6 text-muted">
                  Confirm that these details match your official documents.
                </p>

                <div className="mt-6 rounded-xl border border-primary/10">
                  ...
                  <div className="flex items-start justify-between gap-4 p-4">
                    <div className="min-w-0">
                      ...
                      <p className="mt-2 truncate font-body text-sm text-primary">
                        {primaryDocumentName}
                      </p>
                      <p className="mt-1 truncate font-body text-sm text-primary">
                        {proofOfAddressName}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setKybStage("documents")}
                      className="font-body text-xs font-medium text-primary underline underline-offset-4 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                    >
                      Edit
                    </button>
                ...
                <form onSubmit={(event) => void submitKyb(event)}>
                  {formError ? (
                    <p className="mt-4 rounded-lg border border-red-500/40 bg-red-500/10 px-4 py-3 font-body text-sm font-medium text-red-700">
                      {formError}
                    </p>
                  ) : null}
  ...
  const renderKybTrustStep = (): ReactElement => (
    <main className="fixed inset-0 z-[120] overflow-x-hidden overflow-y-auto bg-surface-soft">
      {renderTopBar(1)}
      <div className="mx-auto min-h-screen w-full max-w-5xl px-5 pb-14 pt-28 sm:px-8">
        <div className="grid items-center gap-8 lg:grid-cols-[minmax(0,1fr)_20rem] lg:gap-12">
          <div>
            <p className="font-body text-xs font-medium uppercase tracking-wide text-accent">
              Rello host verification
            </p>
            <h1 className="mt-3 max-w-2xl font-display text-3xl font-bold leading-tight text-primary sm:text-4xl">
              Help renters know who they&apos;re dealing with.
            </h1>
            <p className="mt-4 max-w-xl font-body text-sm leading-6 text-muted">
              Share your legal details and supporting documents so tenants can
              approach your listings with greater confidence. Reviews typically
              take 1-2 business days.
            </p>

            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <p className="flex items-start gap-3 font-body text-sm text-primary">
                <Check
                  className="mt-0.5 h-4 w-4 shrink-0 text-accent"
                  aria-hidden="true"
                />
                Legal identity reviewed
              </p>
              <p className="flex items-start gap-3 font-body text-sm text-primary">
                <Check
                  className="mt-0.5 h-4 w-4 shrink-0 text-accent"
          ...
          <aside
            className="rounded-xl border border-primary/10 bg-white p-6 shadow-md"
        ...
        <form
          onSubmit={(event) => void submitKyb(event)}
          className="mt-9 rounded-xl border border-primary/10 bg-white p-5 shadow-sm sm:p-8"
        >
          <section
            className="grid gap-5 sm:grid-cols-[2rem_minmax(0,1fr)]"
            aria-labelledby="trust-business-details"
          >
            <span className="flex h-8 w-8 items-center justify-center rounded-full border border-accent font-body text-xs font-medium text-accent">
              1
            </span>
            <div className="min-w-0">
              <p className="font-body text-xs font-medium uppercase tracking-wide text-muted">
              ...
              <h2
                id="trust-business-details"
                className="mt-1 font-display text-2xl font-bold text-primary"
              >
                Tell us who you are
              </h2>
              <p className="mt-2 font-body text-sm leading-6 text-muted">
                Use the same information shown on your official records.
              </p>
              ...
              <fieldset className="mt-6">
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  {(
                    [
                      {
                        description: "For a CAC-registered business.",
                        icon: Building2,
                        label: "Registered Business",
                        value: "REGISTERED",
                      },
                      {
                        description: "For landlords using their legal name.",
                        icon: ShieldCheck,
                        label: "Individual Landlord",
                        value: "INDIVIDUAL",
                      },
                    ...
                    <button
                      ...
                      className={cn(
                        "relative min-h-20 rounded-xl border bg-transparent p-4 pr-10 text-left transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-accent",
                        businessType === value
                          ? "border-accent shadow-sm"
                          : "border-primary/20 hover:border-accent/70",
                      )}
          ...
          <section
            className="mt-8 grid gap-5 border-t border-primary/10 pt-8 sm:grid-cols-[2rem_minmax(0,1fr)]"
            aria-labelledby="trust-supporting-documents"
          >
            <span className="flex h-8 w-8 items-center justify-center rounded-full border border-accent font-body text-xs font-medium text-accent">
              2
            </span>
            <div className="min-w-0">
              ...
              <p className="mt-2 font-body text-sm leading-6 text-muted">
                {businessType === "REGISTERED"
                  ...
                  : businessType === "INDIVIDUAL"
                    ? "Upload a valid government-issued ID and a recent proof of address."
                    : "Choose a business type to confirm the documents required for your profile."}
              </p>
              <div className="mt-5 grid gap-3">
                {documentFields.map((field) => renderDocumentField(field))}
              </div>
          ...
          <div className="mt-8 border-t border-primary/10 pt-6">
            <p className="flex items-start gap-3 font-body text-xs leading-5 text-muted">
              <Lock
                ...
                aria-hidden="true"
              />
              Your information is used for verification and reviewed securely by
              our team.
            </p>
            ...
            <button
              type="submit"
              disabled={!kybReady || isProcessing}
              className="mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-accent px-6 py-3 font-body text-sm font-bold text-primary shadow-sm transition-all duration-200 ease-in-out hover:scale-[1.01] hover:bg-primary hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-accent disabled:cursor-not-allowed disabled:opacity-50 sm:ml-auto sm:flex sm:w-auto sm:min-w-56"
            >
              {isProcessing ? (
                <>
                  <Loader2
  ...
  if (screen === "kyb") {
    const kybDesign = searchParams.get("kybDesign");

    if (kybDesign === "direction-1") {
      return renderKybDossierStep();
    }

    if (kybDesign === "direction-2") {
      return renderKybGuidedStep();
    }

    return renderKybTrustStep();
  }
