...
const LANES = [
  {
    ...
    cases: [
      { id: "DSP-1042", title: "Fixture condition", meta: "Add inspection photos" },
      { id: "DSP-1019", title: "Viewing access delay", meta: "Confirm preferred outcome" },
    ],
  },
  {
    ...
    cases: [
      { id: "DSP-1007", title: "Payment release hold", meta: "Mediator assigned" },
    ],
  },
  {
    title: "Resolved",
    tone: "border border-primary text-primary",
    cases: [
    ...
    <main className="min-h-screen overflow-x-hidden px-5 py-10 sm:px-8 lg:px-10 lg:py-14 xl:px-14">
      <header className="flex flex-col gap-6 pb-8 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div className="rounded-lg border border-accent bg-accent/10 p-5">
          <p className="flex items-center gap-2 font-body text-sm font-bold text-primary">
            <AlertTriangle size={17} className="text-accent-alt" />
      ...
      <section className="grid gap-5 xl:grid-cols-[1fr_21rem]">
        <div className="grid gap-5 lg:grid-cols-3">
          {LANES.map((lane) => (
            <section key={lane.title} className="rounded-lg border border-primary/15 bg-surface-soft p-4 shadow-sm">
              <div className="flex items-center justify-between gap-3">
                <h2 className="font-display text-2xl font-bold text-primary">{lane.title}</h2>
                <span className={`rounded-full px-3 py-2 font-accent text-xs font-bold uppercase tracking-[0.14em] ${lane.tone}`}>
                  {lane.cases.length}
                </span>
              </div>
              <div className="mt-5 space-y-4">
                {lane.cases.map((item) => (
                  <article key={item.id} className="rounded-lg border border-primary/10 bg-[var(--color-bg)] p-5 shadow-sm">
                    <p className="font-accent text-xs font-bold uppercase tracking-[0.18em] text-accent-alt">{item.id}</p>
                    <h3 className="mt-3 font-body text-base font-bold text-primary">{item.title}</h3>
                    <p className="mt-3 font-body text-sm leading-6 text-muted">{item.meta}</p>
        ...
        <aside className="rounded-lg bg-primary p-6 text-white shadow-sm">
          ...
          <div className="mt-7 space-y-4">
            {EVIDENCE.map(({ label, value, icon: Icon }) => (
              <div key={label} className="flex items-center justify-between gap-4 rounded-lg border border-white/15 bg-white/[0.08] p-4">
                <span className="flex items-center gap-3 font-body text-sm font-bold">
                  <Icon size={18} className="text-accent" />
                  {label}
                </span>
                <span className="font-body text-sm text-white/65">{value}</span>
              </div>
            ))}
          </div>
          <button
