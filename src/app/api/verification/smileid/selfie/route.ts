...
async function proxySelfieResponse(
  response: Response,
  backendUrl: string,
): Promise<Response> {
...
export async function POST(request: Request): Promise<Response> {
  ...
  const backendUrl = getBackendUrl("/api/verification/smileid/selfie");
  ...
  try {
    ...
    return proxySelfieResponse(response, backendUrl);
  } catch (error) {
    if (error instanceof Error && error.name === "AbortError") {
      return Response.json(
        {
          success: false,
          message: "Selfie liveness check timed out. Please try again.",
          data: null,
        },
        { status: 504 },
      );
